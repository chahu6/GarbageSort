		...
		<view class="u-tabbar__content safe-area-inset-bottom" :style="{
			...
			<view class="u-tabbar__content__item" v-for="(item, index) in list" :key="index" :class="{
				...
				<view class="u-tabbar__content__item__text">
					<text class="u-line-1" v-if="midColor && item.midColor && midButton && item.midButton" :style="{color: activeColor}">{{item.text}}</text>
					<text class="u-line-1" v-else :style="{color: elColor(index)}">{{item.text}}</text>
				</view>
			</view>
